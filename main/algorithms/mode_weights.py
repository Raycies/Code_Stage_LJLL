...
class GD_estimator:
    ...
    def __init__(self, KL_type: int, p0: np.ndarray, U: list, X: list, h: float, eps: float, max_iter: int, bw: list):
        """
        ...
        self.constant_part = [] # list of $-e^{-U(x_{j,i})}/\hat{\nu}_i(x_{j,i})$ (when KL_type=0), this is a constant coefficient along the descent
        for k in range(self.K):
