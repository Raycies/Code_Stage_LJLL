...
def compute_gaussian_density(X_query, sigma, n_periods=3): # Density of periodised Gaussian of mean 0 and variance sigma^2 I_d
    m, d = X_query.shape
...
def KDE_evaluation(X_train, X_eval, sigma, n_periods=3): # Returns a np.array $G$ of shape (len(X_1)), $G[i] \approx \frac{1}{len(X_2)} \sum_{j=1}^{len(X_2)} g_\sigma(X_1[i]-X_2[j])$
    m, d = X_eval.shape
    n = X_train.shape[0]
...
########========--------- Objective functions, surrogate functions --------========########

"""def estimated_density(X_eval, X_data, p, sigma):
    val = 0
    for k in range(len(p)):
        val += p[k] * KDE_evaluation(X_data[k], X_eval, sigma[k])
    return val

def find_mask(X,Y): # at the end: Xlog(Y). Mask when log(Y) is inaccurate, ie when Y is too low
    return Y >= 1e-200

def KL_right_quadrature(U, X_data, p, sigma, X_quad): # Computes the objective function infinite data
    ...
    estimator_values = estimated_density(X_quad, X_data, p, sigma)
...
def J_infty_right_quadrature(U, nu, p, X_quad): #Computes J_\infty(p)
    ...
    mask = find_mask(exp_part, estimator_values)
    return - np.sum(exp_part[mask] * np.log(estimator_values[mask])) / np.sum(mask) # I removed a /exp_part[mask] so that it is really what the title refers

def true_KL_for_estimator(U, X_data, p, sigma, X_quad): # Computes the True KL (works only when e^-U sums to 1)
    # Numerical issue: The exponential may be very low (e-50), due to numerical error the log term will be worth -inf. To avoid the result being -inf, we select only the indexes such that the exponential term is higher than a threshold.
    exp_part = np.exp(-U(X_quad))
    estimator_values = estimated_density(X_quad, X_data, p, sigma)
    mask = find_mask(exp_part, estimator_values)
...
class GD_estimator:
    ...
    def __init__(self, KL_type, p0, U, X, h, eps, max_iter, bw, n_periods=3):
        # Input: - KL_type, an integer. If 0, it is KL(\mu, \pi) if 1 it is KL(\pi|\mu) (\mu being the target and \pi the estimate)
        #        - $p_0$, the initial value of the weight vector $p$
        ...
        #        - bw is the selector used to compute once and for all the bandwidth used for KDE. FOR THE MOMENT, ONLY SCALAR VALUES AND 'silverman' work.
        # valid values: float (fixed bandwidth), 'silverman', 'cv_ml', 'cv_ls', ('silverman' is the optimal AMISE bandwidth for Gaussian density, 'cv_ml' optimiser of cross validation maximum likelihod, 'cv_ls' optimiser of cross_validation integrated square error. 'silverman' is a good option when the data is unimodal, otherwise 'cv_ml' and 'cv_ls' work rather well, but they take much more bandwidth computation time. Must be a list of what is used for each mode, e.g. with d=3: ['cv_ml', 'silverman', 'cv_ml'].
        ...
        self.n = [0]*self.K # number of data in each cluster (filled in the next for loop)
        self.alpha = [0]*self.K # bandwidths used for KDE (filled in the next for loop)
        self.h = h
        self.U = U
        self.X = X
        ...
        self.start_timer()
        self.KDE_evaluations_on_training = {} # The fact that sigma does not vary allows us to keep only the sum values. self.get_gaussian_sums(i,j) is a numpy array of size n_j, whose q-th component is: \frac{1}{n_i} \sum_{k=1}^{n_i} g_{\sigma_i}(x_{k,i} - x_{q,j})
        for k in range(self.K):
            self.n[k] = len(X[k])
            self.alpha[k] = get_bandwidth(X[k], bw[k], self.n_periods)# KDE in the torus
        ...
        self.constant_part = [] # list of $e^{-U(x_{j,i})}/\hat{\nu}_{\alpha_i}(x_{j,i})$, this is a constant coefficient along the descent
